...
private:
	...
	friend unsigned WINAPI DefThreadProc(LPVOID lpParam);

public:
	ThreadData() : ThreadId(-1), nFPS(60), nLoopCount(-1), LoopID(nullptr), bExit(false), nSleepTime(0), bPlay(true) {}

	ThreadData(int FPS, int LoopCount, void (*FunctionAddress)(void)) : 
		ThreadId(-1),
		nFPS(FPS),
		nLoopCount(LoopCount),
		LoopID(FunctionAddress),
		bExit(false),
		nSleepTime(0),
		bPlay(true) 
	{}
...
public:
	...
	|			  作成した場合、失敗します。
	*********************************************************************************************************************/
	_Check_return_ bool CreateThread(_In_ ThreadData* tData, _In_ _beginthreadex_proc_type ThreadProc, _In_ bool bOnDest, _Out_opt_ LPTHREAD *lpThread);
	...
	****************************************************************************************************************/
	...
	|				  スレッド内で確保した動的メモリ領域は解放されません。その為、メモリリークの危険性があります。
