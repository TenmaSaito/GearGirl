...
void InitResult(void)
{
	...
	SetResult(pos, size, vec3_ZERO, colX_RED, RESULTTEX_RESULT, -1, pResult);

	// "SCORE"
	pos = vec3(SCREEN_WIDTH * 0.4f, SCREEN_HEIGHT * 0.4f, 0.0f);
	size = vec3(SCREEN_WIDTH * 0.2f, SCREEN_HEIGHT * 0.06f, 0.0f);
	SetResult(pos, size, vec3_ZERO, colX_BLUE, RESULTTEX_SCORE, -1, pResult);
	...
	pos = vec3(SCREEN_WIDTH * 0.5f, SCREEN_HEIGHT * 0.15f, 0.0f);
	size = vec3(SCREEN_WIDTH * 0.25f, SCREEN_HEIGHT * 0.1f, 0.0f);
	SetResult(pos, size, vec3_ZERO, colX_WHITE, RESULTTEX_RANK, -1, pResult);
	...
	for (int nCntRank = 0; nCntRank < MAX_NUMSCORE; nCntRank++)
	{
		...
		SetResult(pos, size, vec3_ZERO, colX_WHITE, RESULTTEX_RANKNUM, nCntRank, pResult);
	}
	...
	for (int nCntRank = 0; nCntRank < MAX_NUMSCORE; nCntRank++)
	{
		...
		for (int nCntNum = MAX_SCOREDIGIT - 1; 0 <= nCntNum; nCntNum--)
		{
			...
			{// 今回のスコアの順位の一つ目のアドレス
				g_pThisRanking = SetResult(pos, size, vec3_ZERO, colX_WHITE, RESULTTEX_NUM, nThisScore % 10, pResult);
			}
			else
			{
				SetResult(pos, size, vec3_ZERO, colX_WHITE, RESULTTEX_NUM, nThisScore % 10, pResult);
...
void UpdateRanking(void)
{
	...
	//**************************************************************
	// 前へ
	if (GetKeyboardTrigger(KEY_BACK))
	{
