...
void InitSky(void)
{
	...
	if (FAILED(D3DXCreateTextureFromFile(pDevice, "data\\TEXTURE\\BG\\sky001.jpg", &g_pTextureSky[0])))
		PrintDebugProc(DEBUG_ERROR, "sky001.jpgを読み込めませんでした");
	if (FAILED(D3DXCreateTextureFromFile(pDevice, "data\\TEXTURE\\BG\\sky002.jpg", &g_pTextureSky[1])))
		PrintDebugProc(DEBUG_ERROR, "sky000.jpgを読み込めませんでした");

	//**************************************************************
	// 位置・サイズの初期化
	for (int nCntSky = 0; nCntSky < MAX_SKY; nCntSky++)
	{
		g_aSky[nCntSky].pos = VECTOR3_ZERO;
		g_aSky[nCntSky].rot = VECTOR3_ZERO;
		g_aSky[nCntSky].angle = VECTOR3_ZERO;
